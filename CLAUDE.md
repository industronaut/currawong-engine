...
The central commitment is **sim/view separation**, modelled on UE-style proxy extraction rather than Unity/Godot scene-graph integration. The codebase splits into a few top-level modules:

- `src/sim.rs` + `src/sim/` (submodules `slot_map`, `units`, `facing`, `zone`, `components`, `clock`, `command`, `terrain`, `environment`, `rng`) — sim layer. `sim.rs` is a thin parent that owns the `Simulation` trait and re-exports the submodules' public types so callers see a flat surface. Always compiled. Depends on `glam` (used only at the view-facing seam — sim state is integer), `fixed` (Q16.16 backing for `SimUnit`), `indexmap` (deterministic component / chunk iteration), `rand` + `rand_pcg` (seeded `SimRng`), and `std`. Never imports `wgpu` or `winit`.
...
- `src/grid.rs` — tile-grid topology: the `Grid` trait + `SquareGrid` / `HexGrid` impls. Shared by sim (`Terrain<G>` and `Zone<G>` carry `G` as a phantom-style bound) and the view-side mesher/picker/highlight (which actually invoke the trait methods to walk corners, neighbours, and pick cells from canonical XY). Lives at the crate root because every call site that *uses* the trait methods is view-side — sim only needs the type. Always compiled, depends only on `glam`.
...
- `SimUnit` — Q16.16 fixed-point scalar (`FixedI32<U16>`). Sim arithmetic is integer end-to-end — no `f32` in sim state — for bit-exact determinism across architectures. Range ±32,768 tiles, resolution 1/65,536 tile.
- `SimPos { x, y, z }` and `SimVec { x, y, z }` — position and offset, both three `SimUnit`s. Separate types so `pos + pos` doesn't typecheck; `pos + vec → pos`, `pos - pos → vec`. Tile-flavoured constructors (`SimPos::tile(3, 5, 0)`, `SimPos::tile_center(3, 5, 0)`) for the common case.
- `Facing(u16)` — yaw-only quantized angle. Full circle is `2^16`; arithmetic wraps automatically. `Facing::from_direction(SimVec)` uses an integer atan2 (octant reduction + minimax polynomial) so velocity → facing is bit-exact across platforms with no float trig in the sim. Cardinal helpers (`to_cardinal8`, `to_cardinal16`) on a single right-shift.
- `WorldTransform` — per-object spatial state (`position: SimPos`, `facing: Facing`). The struct is the *transform*; the *object* is the id plus whatever components are attached. Richer payloads live in `Components`, not on the transform. The view-side seam converts to `glam::Vec3` / `glam::Quat` at extract time via `position.to_vec3()` / `facing.to_quat()`.
...
- `Components` — heterogeneous, type-erased registry of sparse per-object state. `IndexMap<TypeId, Box<dyn ComponentStorage>>` outer, `IndexMap<WorldObjectId, T>` per type, lazy-allocated on first `insert::<T>`. APIs: `insert/get/get_mut/remove/iter/iter_mut`, all generic over `T: 'static`. Iteration is in insertion order (deterministic across runs); removal uses `swap_remove` for O(1). Closer to RimWorld's `ThingComp` bag than to archetype ECS — the right shape for sim-game state where most facts are sparse and optional.
...
- `SimRng` — seeded `Pcg64` wrapper, the **only** source of randomness sim code may use. Constructed with `SimRng::from_seed(u64)`; exposes a narrow API (`gen_range_i32`, `gen_range_u32`, `gen_range_sim_unit`, `gen_sub_tile`, `gen_bool`, `shuffle`). Same seed → same sequence, bit-exact across runs.
...
- **Out of scope for the Command landing, deferred to follow-up issues:** replay recording/playback, networking transport, routing internal sim mutation through Commands (Factorio-style unified log). Determinism enforcement landed with #87 — fixed-point `SimUnit`/`SimPos`/`Facing`, ordered `Components`/`Terrain::chunks` via `IndexMap`, and seeded `SimRng`.
...
- **Sim state is integer-only; the view converts at extract.** `SimPos`/`SimVec` (Q16.16 via `fixed`) for position/offset, `Facing(u16)` for yaw-only rotation, `SimUnit` everywhere a scalar would be `f32` (time accumulators, speeds, sub-tile fractions). No `f32` in sim state — that's the precondition for cross-architecture bit-exact replay. The view-side seam (`render_object_pass`, example extract closures, view-side UI labels) calls `position.to_vec3()` / `facing.to_quat()` / `value.to_num::<f32>()` to land in glam-land. Trig outputs that flow only to the view (`sun_direction_for`) keep their float internals — they're not read back into sim logic, so the single-arch cost is acceptable until lockstep multiplayer changes the calculus.
- **Component and chunk iteration is in insertion order.** `Components` (both inner and outer map) and `Terrain::chunks` are `IndexMap`-backed, so iteration is deterministic across runs and across library versions. Removal uses `swap_remove` for O(1) — the last-inserted entry moves into the freed slot, which is still deterministic for a given sequence of operations. Cross-build save-file portability is a separate concern (a stable component-id registry; see #87's out-of-scope).
- **All sim randomness flows through `SimRng`.** No `thread_rng()`, `SystemTime::now()`, `Instant::now()`, or other ambient sources in `src/sim/` or any sim path. `SimRng` (`Pcg64`) is constructed with a 64-bit seed, exposes `gen_range_*`, `gen_sub_tile`, `gen_bool`, `shuffle`, and lives on the user's `Simulation` struct so the seed travels with the sim's identity. Not structurally enforced today — caught by code review and CLAUDE.md, with a CI grep planned. Float trig in sim (`SimEnvironment::sun_direction_for`'s `sin`/`cos`) is the single exception, and only because its output is view-side; if that ever feeds back into sim logic, a fixed-point LUT replaces it.
...
- Tests live in `#[cfg(test)] mod tests` blocks within the file under test, placed in the slice that owns the public API being asserted. Sim-side modules are well-covered (`zone`, `components`, `terrain`, `environment`, `units`, `facing`, `rng`); render-side tests cover what doesn't need a live GPU (`Pod` layout sizes, vertex strides, mip math, CPU downsampling) and the rest is exercised by running examples manually. End-to-end determinism (the load-bearing #87 guarantee) lives in `tests/determinism.rs` — same seed, same hash, every run.
